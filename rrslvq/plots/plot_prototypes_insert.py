...
for x,z in zip(idx1,idx2):
    ...
    ax.set_xlabel('$x_1$')
    ax.set_ylabel('$x_2$')
    red, = plt.plot(p1[:,x],p1[:,z],'rX',markersize=18)
    brown, = plt.plot(X[y==0][:,x],X[y==0][:,z],'b.')
    green,= plt.plot(X[y==1][:,x],X[y==1][:,z],'g.')
    plt.box(False)
    plt.axis('off')
    plt.legend([brown, green,red], ["Class 1", "Class 2","Prototypes"])
    # ax.set_title('Insert Step')
    fig.savefig("insert.pdf",dpi=400, format='pdf',bbox_inches='tight')
    ...
    brown, = plt.plot(X[y==0][:,x],X[y==0][:,z],'b.')
    green, plt.plot(X[y==1][:,x],X[y==1][:,z],'g.')
    brown_prot, = plt.plot(p2[p2l==0][:,x],p2[p2l==0][:,z],'bX',markersize=18)
    green_prot, = plt.plot(p2[p2l==1][:,x],p2[p2l==1][:,z],'gX',markersize=18)
    plt.box(False)
    plt.axis('off')
    plt.legend([brown, green,brown_prot,green_prot], ["Class 1", "Class 2","Prototypes Class 1","Prototypes Class 2"])
    fig.savefig("optimization.pdf",dpi=400, format='pdf',bbox_inches='tight')
plt.show()
