function [Kr] = tca( sourcefeatures,targetfeatures,m,sigma,kernelType)
...
%   output: resultant kernel matrix
    %% error
    test = size(sourcefeatures,2);
    if m >= size(sourcefeatures)
        error 'm is larger than the total number of features';
    end
    %% compute K
    Kss = kernel(kernelType,sourcefeatures',sourcefeatures',sigma);
    Kst = kernel(kernelType,sourcefeatures',targetfeatures',sigma);
    Kts = kernel(kernelType,targetfeatures',sourcefeatures',sigma);
    Ktt = kernel(kernelType,targetfeatures',targetfeatures',sigma);
    K = [[Kss,Kst];[Kts,Ktt]];
