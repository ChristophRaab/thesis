function [acc_data,time_data,nsvds_data] = approaches(Xs,Ys,Xt,Yt,options,acc_data,time_data,nsvds_data)
%APPROACHES Summary of this function goes here
...
% %     SVM
    ...
    nsvds_data = [nsvds_data model.totalSV];
% %     
%     %% TCA
    tic
    nt = length(Ys);
...
% % %     BT GA 
    tic;
    [model,K,m,n] = cl_bt_ga(Xs,Xt,Ys,options);
    [label, acc,scores] = svmpredict(full(Yt), [(1:n)', K(m+1:end, 1:m)], model);
    fprintf('\n BT GA %.2f%% \n', acc(1));
    time_data = [time_data toc];
    acc_data = [acc_data acc(1)];
...
% % BT LGA
    tic;
    [model,K,m,n] = cl_bt_lga(Xs,Xt,Ys,options);
    [label, acc,scores] = svmpredict(full(Yt), [(1:n)', K(m+1:end, 1:m)], model);
    fprintf('\n BT LGA %.2f%% \n', acc(1));
    time_data = [time_data toc];
    acc_data = [acc_data acc(1)];
