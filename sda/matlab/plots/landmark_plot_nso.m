...
%              Xs-SCORE and Sampling
            nsoureIndx = crossvalind('Kfold', Ys, 2);
            targetIndx = crossvalind('Kfold', Yt,2);
            Xs = Xs(find(nsoureIndx==1),:);
            Ys = Ys(find(nsoureIndx==1),:);
...
save(strcat("image_error_vs_landmarks_nso.mat"),'result');
load image_error_vs_landmarks_nso.mat
...
print("image_error_vs_landmarks_nso",'-depsc','-r400')
...
result = [landmarks; result];
save(strcat("text_error_vs_landmarks_nso.mat"),'result');
load text_error_vs_landmarks_nso.mat
result = result';
x = result(:,1);
v = mean(result(:,2:end)');
xq = [1:1000,0.1];
...
ax_height = outerpos(4) - ti(2) - ti(4);
ax.Position = [left bottom ax_width ax_height];
print("text_error_vs_landmarks_nso",'-depsc','-r400')
...
result = [landmarks; result];
save(strcat("decaf_error_vs_landmarks_nso.mat"),'result');
load decaf_error_vs_landmarks_nso.mat
...
ax_height = outerpos(4) - ti(2) - ti(4);
ax.Position = [left bottom ax_width ax_height];
print("decaf_error_vs_landmarks_nso",'-depsc','-r400')
