...
%             
% % % % %             SO
%            [model,K,m,n] = cnso(Xs,Xt,Ys,options);
...
% save(strcat("image_error_vs_landmarks_cnso.mat"),'result');
load image_error_vs_landmarks_cnso.mat
...
print("image_error_vs_landmarks_cnso",'-depsc',"-r400")
...
% end
% result = [landmarks; result];
% save(strcat("text_error_vs_landmarks_cnso.mat"),'result');
load text_error_vs_landmarks_cnso.mat
result = result';
x = result(:,1);
v = mean(result(:,2:end)');
xq = [1:1000,0.1];
...
ax_height = outerpos(4) - ti(2) - ti(4);
ax.Position = [left bottom ax_width ax_height];
print("text_error_vs_landmarks_cnso",'-depsc','-r400')
...
% end
% result = [landmarks; result];
% save(strcat("decaf_error_vs_landmarks_cnso.mat"),'result');
load decaf_error_vs_landmarks_cnso.mat
...
ax_height = outerpos(4) - ti(2) - ti(4);
ax.Position = [left bottom ax_width ax_height];
print("decaf_error_vs_landmarks_cnso",'-depsc','-r400')
